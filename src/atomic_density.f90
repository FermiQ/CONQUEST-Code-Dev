...
module atomic_density
   ...
   type radial_density
      integer :: length ! Length of the radial density table
      real(double) :: cutoff ! Cutoff radius for the radial density
      real(double), pointer, dimension(:) :: table ! Radial density table
      real(double), pointer, dimension(:) :: d2_table ! Second derivative of the radial density table
 
      ! for Neutral atom potential
      real(double) :: delta ! Delta parameter for neutral atom potential
      integer :: k_length ! Length of the k-space table for neutral atom potential
      real(double) :: k_delta ! Delta k for neutral atom potential
      real(double), pointer, dimension(:) :: k_table ! k-space table for neutral atom potential
   end type radial_density
 
   type(radial_density), allocatable, dimension(:) :: atomic_density_table ! Array of radial density tables for each atomic species
 
   ! Maximum cutoff atomic on charge density tables
   real(double), allocatable, dimension(:) :: rcut_dens ! Array of maximum cutoff radii for atomic charge density tables
